...
给 AI 协作项目的开发流程地基：`npx bosscoding init` 一条命令装好规则文件、8 项守卫、CI 质检口、决策档案与技能，任何 coding agent 通吃。npm 包名 `bosscoding`，纯 Node ESM，运行时零第三方依赖。尚未发布、仓库私有（公开与发包须老板确认）。
...
- `npm publish` 发包、仓库转公开、以老板名义对外发布任何内容——这三件是本项目的「上线」，一律先确认。
- 任何花钱的操作；动权限、密钥、账号设置。
