...
2. 从 main（稳定版本）开任务分支。并行的第二个任务起，用项目选定的包管理器执行 `bosscoding task <任务名>`，准备独立工作区；不要让老板切目录、搬文件或清理工作区。
...
7. 本地阶段（`git remote -v` 为空）：先把改动提交在任务分支。老板说「验收通过，执行 BossCoding 收尾」后，在当前任务工作区用项目选定的包管理器执行 `bosscoding finish`；不要自己 `checkout main`，也不要让老板执行命令。
