...
export function createContext(root = process.cwd()) {
  ...
  const ctx = {
    ...
    /**
     * 工作区里「算数」的文件：已追踪 ＋ 未追踪但没被忽略的。
     *
     * 为什么需要它（实测事故）：全新项目里一切都还没提交，只看已追踪文件的守卫
     * 会对一个躺在工作区里的明文密钥报「全部通过」——而 init 恰恰教新人此刻跑一次检查。
     * 那个绿是「什么都没看」的绿，教会新人的第一课是错的。
     * 被 .gitignore 忽略的不算（node_modules 之类），否则扫描面会炸。
     */
    workingFiles() {
      ...
      const untracked = execFileSync("git", ["ls-files", "-z", "--others", "--exclude-standard"], {
        ...
        .split("\0")
        .filter(Boolean);
      workingCache = [...new Set([...ctx.trackedFiles(), ...untracked])];
      return workingCache;
    },
