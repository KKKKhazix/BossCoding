...
export function runCheck(root = process.cwd()) {
  ...
  if (!ctx.isGitRepo()) {
    ...
    console.error("  修：新项目运行 npx guiju init（会顺带 git init）；已有项目先 git init。");
    return 1;
  }
      ...
      problems = [
        {
          ...
          fix: "这是 guiju 的问题不是你的问题，请到 guiju 仓库开 issue 附上本条输出。",
        },
      ];
    }
  ...
  console.log(paint.red(`${failed}／${total} 项守卫未过。按上面的「修」逐条处理后重跑 npx guiju check。`));
  return 1;
}
