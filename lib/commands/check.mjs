...
function isGitTopLevel(root) {
  try {
    ...
    return fs.realpathSync(top) === fs.realpathSync(path.resolve(root));
  } catch {
    return false;
  }
}
...
export function runCheck(root = process.cwd()) {
  ...
  if (!ctx.isGitRepo()) {
    ...
    console.error("  修：新项目运行 npx -y bosscoding@latest init（会顺带创建版本记录）；已有项目让 AI 先建版本记录。");
    return 1;
  }
  if (!isGitTopLevel(ctx.root)) {
    console.error(paint.red("✗ 当前在产品项目的子文件夹里，不能从这里给整个项目判绿。"));
    console.error("  把这句话交给 AI：「回到这个版本库的最顶层目录，再运行 npx bosscoding check。」");
    return 1;
  }
