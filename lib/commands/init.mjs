...
export function runInit(root = process.cwd()) {
  ...
  write(".agents/skills/boss-flow/SKILL.md", skillBody);
  const claudeSkillDir = path.join(abs, ".claude", "skills");
  const claudeSkill = path.join(claudeSkillDir, "boss-flow");
  if (!fs.existsSync(claudeSkill)) {
    fs.mkdirSync(claudeSkillDir, { recursive: true });
    try {
      fs.symlinkSync(path.join("..", "..", ".agents", "skills", "boss-flow"), claudeSkill, "junction");
      done.push(".claude/skills/boss-flow（软链）");
    } catch {
      fs.mkdirSync(claudeSkill, { recursive: true });
      fs.writeFileSync(path.join(claudeSkill, "SKILL.md"), skillBody);
      done.push(".claude/skills/boss-flow（复制，本系统不支持软链）");
    }
  } else {
    skipped.push(".claude/skills/boss-flow");
  }
  ...
  // 8. git hook：主工作区只跑 main（单工作区的项目里它一声不吭，见 templates/hooks/）。
  const hooks = installHooks(abs);
  if (hooks.installed.length > 0) done.push(`git hook：${hooks.installed.join("、")}（主工作区只跑 main）`);
  if (hooks.skipped.length > 0) {
    notes.push(
      `已有别人的 git hook（${hooks.skipped.join("、")}），未覆盖：想要「主工作区只跑 main」的保护，请把 ${path.join(TEMPLATES, "hooks", "main-worktree.sh")} 的内容并进你现有的 hook。`,
    );
  }
  ...
  console.log(`
下一步（给 AI 或老板）：
  1. npm install          安装依赖（含 bosscoding 本体）
  2. npx boss check       守卫应当全绿
  3. 打开 AGENTS.md，把开头的项目简介填成三句大白话
  4. 对你的 AI 说：「读一遍 AGENTS.md。之后需求我说，规矩你守。」${inRepo ? "" : "\n  （仓库刚刚初始化，别忘了首个 commit）"}`);
  return 0;
}
