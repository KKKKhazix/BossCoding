...
import { DEFAULT_PREFLIGHT, ensurePreflightScripts } from "../preflight.mjs";
...
function directoryState(abs) {
  ...
  const looksLikeProject = entries.some((entry) => {
    ...
    if (entry.isDirectory() && SOURCE_DIRS.has(entry.name)) return true;
...
export function runInit(root = process.cwd()) {
  ...
  if (!fs.existsSync(abs) || !fs.statSync(abs).isDirectory()) {
    ...
    console.error('  把这句话交给 AI：「请新建一个只放这个产品的文件夹，在里面重新运行 npx -y bosscoding@latest init。」');
    return 1;
  }
  ...
  try {
    ...
    if (error?.code === "ENOENT") {
      ...
      console.error('  把这句话交给 AI：「请帮我安装 Git，装好后在这个文件夹重新运行 npx -y bosscoding@latest init。」');
      return 1;
    }
  }
  ...
  if (!state.empty && !state.looksLikeProject && !inRepo) {
    ...
    console.error('  把这句话交给 AI：「请新建一个只放这个产品的文件夹，再在新文件夹运行 npx -y bosscoding@latest init。」');
    return 1;
  }
  ...
  if (fs.existsSync(pkgPath)) {
    try {
      ...
      console.error('  把这句话交给 AI：「请修好 package.json，再重新运行 npx -y bosscoding@latest init。」');
      return 1;
    }
  }
  ...
  if (!inRepo) {
    try {
      ...
      console.error('  把这句话交给 AI：「请检查 Git 为什么无法在这个文件夹工作，修好后重新运行 npx -y bosscoding@latest init。」');
      return 1;
    }
  }
  ...
  if (existingPackage === null) {
      ...
      `${JSON.stringify(
        {
          ...
          version: "0.1.0",
          private: true,
          scripts: { test: "node --test", preflight: DEFAULT_PREFLIGHT },
    ...
    if (ensurePreflightScripts(pkg.scripts)) touched = true;
    pkg.devDependencies ??= {};
  ...
  if (!write("AGENTS.md", readTemplate("AGENTS.template.md"))) {
    const existing = fs.readFileSync(path.join(abs, "AGENTS.md"), "utf8");
    if (!hasBossCodingRules(existing)) {
      notes.push(
        `AGENTS.md 是你原有的规则，已保护不覆盖。请把这整句话交给 AI：「保留 AGENTS.md 里的全部现有规则；读取 ${path.join(TEMPLATES, "AGENTS.template.md")}，把缺少的 BossCoding 规则合并进去，不要整份覆盖；完成后运行 npm install 和 npm run preflight，修到通过。」`,
      );
    }
  }
  ...
  console.log(`
下一步只做一件事：
  把这句话交给 AI：「${notes.length > 0 ? "先替我逐条完成上面的『需要处理』；" : ""}运行 npm install 和 npm run preflight，修到通过；读一遍 AGENTS.md，之后需求我说、规矩你守。完成后只问我：『你想做个什么？』」${inRepo ? "" : "\n  （版本记录刚建好，第一次保存也交给 AI，你不用管。）"}

任何时候不知道自己在哪一步：让 AI 运行 npx -y bosscoding@latest status`);
  return 0;
}
