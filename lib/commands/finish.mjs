...
import { isGitHubRemote } from "../git-remote.mjs";
import { detectPackageManager, packageManagerCommand } from "../package-manager.mjs";
import { customPreflightScript } from "../preflight.mjs";
import { readPackageState, testEntryState } from "../project-health.mjs";
...
function defaultBase(runner, root) {
  const configured = tryGit(runner, root, [
    "config",
    "--local",
    "--get",
  ...
  if (
    configured &&
    tryGit(runner, root, ["show-ref", "--verify", "--quiet", `refs/heads/${configured}`]) !== null
  ) {
    return configured;
  }
  ...
  const candidates = (tryGit(runner, root, [
    "for-each-ref",
...
function qualityConfig(runner, root) {
  const files = (tryGit(runner, root, ["ls-files", "--cached", "--others", "--exclude-standard"]) ?? "")
    .split(/\r?\n/)
    .filter(Boolean);
  const state = testEntryState(root, files, readPackageState(root));
  return {
    testReady: state.testEntryConfigured,
    customScript: customPreflightScript(state.scripts),
  };
}
...
export async function runFinish(root = process.cwd(), options = {}) {
  ...
  if (!base) {
    console.error(paint.red("✗ 找不到项目明确使用的稳定分支，不能安全决定合并目标。"));
    console.error("  把这句话交给 AI：「确认项目主干叫什么，再处理这条任务的收尾。」");
    return 1;
  }
  if (!branch) {
    console.error(paint.red("✗ 这里没有明确的任务分支，不能判断要收哪条任务。"));
    console.error("  把这句话交给 AI：「找到这条任务所在的分支工作区，再运行 npx bosscoding finish。」");
    return 1;
  }
  if (branch === base) {
    console.error(paint.red(`✗ 这里是稳定分支 ${base}，不是待收尾的任务分支。`));
    console.error("  把这句话交给 AI：「找到刚才完成并验收的任务分支，再运行 npx bosscoding finish。」");
    return 1;
  }
  ...
  const firstTaskInOnlyWorkspace =
    !main &&
    entries.length === 1 &&
    task?.branch === branch &&
    tryGit(runner, taskRoot, ["show-ref", "--verify", "--quiet", `refs/heads/${base}`]) !== null;
  if ((!main && !firstTaskInOnlyWorkspace) || !task) {
    console.error(paint.red(`✗ 找不到占用 ${base} 的主工作区，不能安全合并。`));
    console.error("  把这句话交给 AI：「恢复主工作区到主干，再重新运行 npx bosscoding finish。」");
    return 1;
  }
  const stableRoot = main?.path ?? taskRoot;
  const sameWorkspace = !main;

  const roots = [...new Set([taskRoot, stableRoot])];
  const dirty = roots.filter((dir) => !isClean(runner, dir));
  if (dirty.length > 0) {
  ...
  const packageState = readPackageState(taskRoot);
  const manager = detectPackageManager(taskRoot, packageState.pkg);
  if (manager.ambiguous) {
    console.error(paint.red("✗ 项目同时出现多套安装工具的痕迹，主干没有改动。"));
    console.error("  把这句话交给 AI：「确认项目原来用哪一种包管理器，只保留正确声明和一套锁文件，再重新收尾。」");
    return 1;
  }
  const runScript = (script) => {
    const invocation = packageManagerCommand(manager, "run-script", { script });
    return command(runner, taskRoot, invocation.command, invocation.args, "inherit");
  };

  // 先钉住「即将接受检验的那一版」。此前在测试结束到读取 taskSha 之间，
  // 另一个进程若恰好提交了一笔，新提交会保持工作区干净并被直接合入——
  // 等于把没跑过产品测试的菜夹进了已经验过的盘子。
  const testedTaskSha = git(runner, taskRoot, ["rev-parse", branch]);
  const taskRevisionUnchanged = () =>
    tryGit(runner, taskRoot, ["rev-parse", branch]) === testedTaskSha &&
    tryGit(runner, taskRoot, ["rev-parse", "HEAD"]) === testedTaskSha;
  const revisionChanged = () => {
    console.error(paint.red("✗ 任务版本在自检期间发生了变化，主干没有改动。"));
    console.error("  把这句话交给 AI：「确认没有别的进程继续提交这条任务，再对最新版本重跑 BossCoding 收尾。」");
    return 1;
  };
  ...
  try {
    ...
    if (quality.customScript) {
      runScript(quality.customScript);
      if (!taskRevisionUnchanged()) return revisionChanged();
    }
    runScript("test");
    if (!taskRevisionUnchanged()) return revisionChanged();
    ...
    console.error("  把这句话交给 AI：「用项目原来的安装工具修好完整自检，再重新运行 BossCoding 收尾。」");
    return 1;
  }

  // 测试脚本也可能意外改文件；真正合并前再看一遍，绿灯不能覆盖脏现场。
  if (roots.some((dir) => !isClean(runner, dir))) {
  ...
  if (!taskRevisionUnchanged()) return revisionChanged();
  const taskSha = testedTaskSha;
  const baseSha = git(runner, stableRoot, ["rev-parse", base]);
  ...
  if (sameWorkspace) {
    try {
      command(runner, stableRoot, "git", ["checkout", base]);
    } catch {
      console.error(paint.red(`✗ 无法安全切回 ${base}，主干没有合并任务版本。`));
      console.error("  把这句话交给 AI：「检查为什么不能回到稳定版本，处理后重跑 npx bosscoding finish。」");
      return 1;
    }
  }
  try {
    command(runner, stableRoot, "git", ["merge", "--ff-only", taskSha]);
  } catch {
  ...
  const mergedSha = git(runner, stableRoot, ["rev-parse", base]);
