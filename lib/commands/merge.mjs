...
function findToken(root) {
  ...
  try {
    return git(root, ["config", "--get", "bosscoding.token"]) || null;
  } catch {
    return null;
  }
}
...
async function fetchPullsFromGitHub({ owner, repo, token }) {
  ...
  if (!res.ok) {
    ...
    const hint =
      res.status === 404
        ? "仓库不存在或是私有仓库——私有仓库要令牌：设环境变量 GITHUB_TOKEN，或 git config bosscoding.token <令牌>。"
        : res.status === 403
...
export async function runMerge(root = process.cwd(), options = {}) {
  ...
  let remote = originUrl ? parseRemote(originUrl) : null;
  // 兜底：GitHub Actions 里 GITHUB_REPOSITORY 恒为 owner/repo，比任何地址解析都准；
  // 本地仓库的 origin 指向镜像或代理时，它也是唯一能用的身份来源。
  if (!remote && process.env.GITHUB_REPOSITORY?.includes("/")) {
  ...
  if (!remote) {
    if (!originUrl) {
      ...
      console.log("  分支自检（npm run preflight）绿了就可以直接在本地合并回主干。");
      console.log(paint.dim("  想要异地备份＋云端质检口：对你的 AI 说「带我连上 GitHub」（四阶梯第 1 阶）。"));
      return 0;
    }
    console.error(paint.red("✗ 认不出这是哪个 GitHub 仓库，排队判定用的是 GitHub 的 PR 号。"));
    console.error("  修：确认 git remote -v 里的 origin 指向 github.com；");
    console.error("      origin 指向镜像或代理时，设环境变量 GITHUB_REPOSITORY=<owner>/<仓库名> 即可。");
    return 1;
  }
  ...
  console.log(paint.dim("  本命令只做判定，不替你转 Ready、不替你合并。"));
  return 0;
}
