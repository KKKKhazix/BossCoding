 ...
 *
 * 两个环境入口外，再优先读取 GitHub 官方登录工具（小白路径必踩）：四阶梯教新人建**私有**仓库，
...
async function fetchPullsFromGitHub({ owner, repo, token }) {
  ...
  if (!res.ok) {
    ...
    const hint =
      res.status === 404
        ? "仓库不存在，或者是私有仓库而当前 GitHub 登录没有访问权。让 AI 检查仓库地址并带你完成 GitHub 登录；账号授权必须由你确认。"
        : res.status === 403
          ? "查询过快或当前账号没有权限。让 AI 检查 GitHub 登录与仓库权限；不要把通行凭据写进项目配置。"
          : res.status === 401
            ? "GitHub 登录已失效。让 AI 带你重新登录；账号授权必须由你确认。"
...
export async function runMerge(root = process.cwd(), options = {}) {
  ...
  let pulls;
  try {
    pulls = await fetchPulls({ ...remote, token: findToken() });
  } catch (error) {
