...
export async function runMerge(root = process.cwd(), options = {}) {
  ...
  try {
    ...
    console.error(paint.red("✗ 读不到 origin 远端——本命令要问 GitHub「谁在排队」。"));
    console.error("  修：在 git 仓库里运行；确认 git remote -v 有 origin。");
    return 1;
  }
  if (!remote) {
    console.error(paint.red("✗ origin 不是 GitHub 仓库，排队判定用的是 GitHub 的 PR 号。"));
    console.error("  修：本命令只支持 GitHub；其他平台请继续按规则文件里的约定人工确认。");
    return 1;
  }
