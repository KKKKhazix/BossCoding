...
const SKILLS = ["boss-flow", "boss-ladder"];
...
export function probe(root) {
  ...
  const dependencies = dependencyState(abs, packageState);
  let entries = [];
  if (isRepo) {
    try {
  ...
  const taskWorktrees = entries.filter((entry) => entry.branch?.startsWith("lane/"));
  const branch = isRepo ? tryGit(abs, ["symbolic-ref", "--short", "HEAD"]) : null;
  const files = projectFiles(abs, isRepo);
  const taskBranch = branch?.startsWith("lane/") ?? false;
  ...
  const stableBranch = taskBranch ? defaultBase(abs) : null;
...
export function runStatus(root = process.cwd()) {
  ...
  const activeTasks = s.taskWorktrees.filter((entry) => !stalePaths.has(path.resolve(entry.path)));
  ...
  if (!s.remoteConfigured) {
    ...
    console.log(paint.yellow("  已连接 GitHub，但当前版本还没有上传记录") + `：${s.originDisplay}`);
