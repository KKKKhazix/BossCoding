...
export function detectPackageManager(root, pkg = null) {
  ...
  return {
    ...
    configs: entries.filter((item) => item.kind === "config").map((item) => item.file),
    ambiguous,
  };
}
...
export function packageManagerCommand(manager, action, options = {}) {
  ...
  if (action === "install") {
    ...
    if (name === "yarn") {
      ...
      const modern =
        (Number.isFinite(major) && major >= 2) ||
        (manager?.configs ?? []).includes(".yarnrc.yml");
      const frozenFlag = modern ? "--immutable" : "--frozen-lockfile";
      return {
        command: "yarn",
