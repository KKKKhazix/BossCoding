  ...
  run(ctx) {
    ...
    if (!ctx.exists(".gitignore")) {
      problems.push({
        ...
        fix: "运行 npx bosscoding init 会自动补上标准忽略规则；或手动创建 .gitignore 并加入 .env、.env.local、.env.*.local 三行。",
      });
      return problems;
    }
    ...
    if (missing.length > 0) {
      problems.push({
        ...
        fix: "在 .gitignore 里为缺失项各加一行（或用一行 `.env*` 全覆盖）；npx bosscoding init 也会自动补齐。",
      });
    }
    return problems;
  },
