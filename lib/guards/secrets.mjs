/**
 * 密钥守卫：扫描工作区里所有算数的文本文件（已追踪 ＋ 未追踪未忽略），抓高置信度的密钥模式。
 *
 * 为什么连没提交的也扫（实测事故）：全新项目里一切都还没进版本库，只看已追踪文件时
 * 一把躺在工作区的明文密钥会被报成「全部通过」。密钥这一类的危害在于「写下来了」，
 * 不在于「提交了」——等提交才报，已经晚了一步。CI 上跑的是干净检出，没有未追踪文件，
 * 所以这次扩面只让本地更早发现问题，不会把任何人的 CI 从绿改红。
 *
...
export default {
  ...
  run(ctx) {
    ...
    for (const file of ctx.workingFiles()) {
      // 未追踪的密钥文件由 .gitignore 覆盖与否决定死活：没被忽略就是「随时会被提交」。
      if (SENSITIVE_FILE.test(file) && !SAFE_ENV_TEMPLATE.test(file)) {
        problems.push({
          file,
          msg: tracked.has(file)
            ? "疑似密钥文件被 git 追踪（一旦 push 到远端，删除也救不回来）"
            : "疑似密钥文件没有被 .gitignore 忽略，下次 git add 就会被收进版本库",
