 ...
 * - 确属误报的行，行内加注释 `guiju-allow-secret` 放行；
...
const ALLOW_MARK = "guiju-allow-secret";
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        for (const [rule, regex] of RULES) {
          ...
          problems.push({
            ...
            fix: "真值挪进 .env（.gitignore 已忽略它），代码里改读 process.env；已 push 过的密钥必须轮换。确属误报时在该行加注释 guiju-allow-secret。",
          });
          break;
        }
      }
