...
export function installHooks(root = process.cwd(), options = {}) {
  ...
  try {
    ...
    if (stat !== null && !stat.isDirectory()) {
      result.blocked.push(
        "Git hook 路径不是项目内的普通文件夹，BossCoding 没有写入本地门禁。请把这句话交给 AI：「请修复本项目的 Git hook 路径，再运行 BossCoding 更新。」",
      );
      return result;
    }
    ...
    result.blocked.push(
      "Git hook 文件夹无法安全创建，BossCoding 没有写入本地门禁。请把这句话交给 AI：「请检查本项目的 Git hook 路径和文件权限，再运行 BossCoding 更新。」",
    );
    return result;
  }
